# DS 4420 Final Project README 

### Group Members 
Emma Penn, Janina Kurowski, and Tessa Joseph
...
We apply **two machine learning methods** — one in Python, one in R — with at least one Bayesian model, per course requirements. At least one method is implemented manually without pre-built modeling packages.
...
Key steps:
1. Filtered CMS dataset to DRG codes for female reproductive system disorders
...
The ACF plot did not exhibit a strong cutoff, indicating the series may lack the well-defined autocorrelation structure needed for a standard ARIMA-type model. Key limitations:

- The data is **not updated quarterly**, limiting the number of usable time points
- Available aggregate data runs only through 2013 in this dataset, which is insufficient for a robust long-horizon time series
- The signal for reproductive disorder discharges specifically may be too sparse at the national aggregate level to model with confidence
...
```bash
cd phase1/
python time_series_poc.py
```

Outputs: time series plot of annual discharge counts and ACF plot for the filtered reproductive disorder DRG series.
